...
void log_packet(const AVFormatContext *fmt_ctx, const AVPacket *pkt) {
  ...
  char pts_str[AV_ERROR_MAX_STRING_SIZE];
  char dts_str[AV_ERROR_MAX_STRING_SIZE];
  char duration_str[AV_ERROR_MAX_STRING_SIZE];

  av_ts_make_string(pts_str, pkt->pts);
  av_ts_make_string(dts_str, pkt->dts);
  av_ts_make_string(duration_str, pkt->duration);

  char pts_str_tb[AV_ERROR_MAX_STRING_SIZE];
  char dts_str_tb[AV_ERROR_MAX_STRING_SIZE];
  char duration_str_tb[AV_ERROR_MAX_STRING_SIZE];

  av_ts_make_time_string(pts_str_tb,pkt->pts,time_base);
  av_ts_make_time_string(dts_str_tb,pkt->dts, time_base);
  av_ts_make_time_string(duration_str_tb,pkt->duration,time_base);

  logging("pts:%s pts_time:%s dts:%s dts_time:%s duration:%s duration_time:%s stream_index:%d",
          pts_str, pts_str_tb,
          dts_str, dts_str_tb,
          duration_str, duration_str_tb,
          pkt->stream_index);
}
...
void print_timing(char *name, AVFormatContext *avf, AVCodecContext *avc, AVStream *avs) {
  ...
  if (avc != NULL) {
    logging(
        "\t\tbit_rate=%d ticks_per_frame=%d width=%d height=%d gop_size=%d keyint_min=%d sample_rate=%d profile=%d level=%d ",
        avc->bit_rate,
        avc->ticks_per_frame,
        avc->width,
        avc->height,
