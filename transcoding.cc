...
//  av_opt_set(sc->video_avcc->priv_data, "preset", "fast", 0);
  ...
  av_opt_set(sc->video_avcc->priv_data, "preset", "ultrafast", 0);
  ...
  av_opt_set(sc->video_avcc->priv_data, "crf", "35", 0);
  ...
  av_opt_set(sc->video_avcc->priv_data, "tune", "zerolatency", 0);
...
int encode_video(StreamingContext *decoder, StreamingContext *encoder, AVFrame *input_frame) {
  ...
  while (response >= 0) {
    ...
    if (response == AVERROR(EAGAIN) || response == AVERROR_EOF) {
      ...
      av_make_error_string(err_str, AV_ERROR_MAX_STRING_SIZE, response);
      logging("Error while receiving packet from encoder: %s", err_str);
      return -1;
    }

    output_packet->stream_index = decoder->video_index;
    output_packet->duration =
        encoder->video_avs->time_base.den / encoder->video_avs->time_base.num / decoder->video_avs->avg_frame_rate.num
    ...
    av_make_error_string(err_str, AV_ERROR_MAX_STRING_SIZE, response);
    if (response != 0) {
      logging("Error %d while receiving packet from decoder: %s", response, err_str);
      return -1;
    }
  }
...
int transcode_audio(StreamingContext *decoder,
  ...
  if (response < 0) {
    char err_str[AV_ERROR_MAX_STRING_SIZE];
    av_make_error_string(err_str, AV_ERROR_MAX_STRING_SIZE, response);
    logging("Error while sending packet to decoder: %s", err_str);
    return response;
  }
  ...
  while (response >= 0) {
    ...
    if (response == AVERROR(EAGAIN) || response == AVERROR_EOF) {
      ...
      char err_str[AV_ERROR_MAX_STRING_SIZE];
      av_make_error_string(err_str, AV_ERROR_MAX_STRING_SIZE, response);
      logging("Error while receiving frame from decoder: %s", err_str);
      return response;
    }
